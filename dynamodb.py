...
def write_to_ddb(key, data):
 ...
 How To Set Time-To-Live(TTL) Attribute For Amazon DynamoDB entries? February 20, 2019   3 minute read  
 Menu
Possible use cases
Setup
1 AWS CLI
...
In case you wonder what epoch means, the definition of Epoch on Google Dictionary is a period of time in history or a person's life, typically one marked by notable events or particular characteristics.

Possible use casesPermalink
Keep only relatively new data for debugging or analytics.
Expire users’ periodic usage data so that usage quota can be reset at a specific time.
Keep data temporary for pattern searching and usage tracking.
SetupPermalink
...
The following modal will show up, you can write your attribute name in the TTL attribute input. AWS DyanamoDB Enable TTL
Python Code to set ttlPermalink
You can refer to AWS Boto3 DynamoDB document for AWS DynamoDB APIs

Write to DDBPermalink
import datetime
import time
import boto3
